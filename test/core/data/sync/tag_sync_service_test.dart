...
void main() {
  ...
  test('auto-created straggler tag is overwritten by its real definition',
      () async {
    // A note arrives before its tag definition: setTagsForNote auto-creates
    // a stub "work". Its lastModified must NOT shadow the real definition.
    final authorId = await db
        .into(db.authors)
        .insert(AuthorsCompanion.insert(accountName: 'a'));
    final noteId = await db
        .into(db.notes)
        .insert(NotesCompanion.insert(subject: 's', authorId: authorId));
    await repo.setTagsForNote(noteId, ['work']); // auto-creates stub

    // The real definition (with a real-but-earlier-than-now timestamp) arrives.
    await svc.mergeDefinitions(
      {'tags': [_tag('work', desc: 'real', lm: '2026-05-01T00:00:00Z')]},
      deviceId: 'dev', now: DateTime.utc(2026, 6, 1),
    );

    final work =
        (await repo.getTagsWithMeta()).firstWhere((t) => t.name == 'work');
    expect(work.description, 'real'); // real def applied, not shadowed by stub
  });
}
