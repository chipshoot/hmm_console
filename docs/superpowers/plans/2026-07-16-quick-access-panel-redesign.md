...
void main() {
  ...
  test('Home is a simple action; Sync is a custom action rendering SyncPill',
      () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    final actions = container.read(quickPanelActionsProvider);
    ...
    // The Sync builder returns a const SyncPill and must not dereference
    // ctx/ref, so the fakes are never actually called.
    final built = sync.builder!(_FakeContext(), _FakeRef());
    expect(built, isA<SyncPill>());
  });
}
```
